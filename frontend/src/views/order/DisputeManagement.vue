<template>
  <n-space vertical>
    <n-h2>纠纷管理</n-h2>
    <n-text depth="3">管理纠纷订单，查看买卖双方陈述并做出仲裁裁决</n-text>
        ...
        <n-space align="center" justify="space-between">
          ...
          <n-space align="center">
            <n-image
              ...
              fallback-src="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect fill='%23e0e0e0' width='64' height='64'/></svg>"
            />
            <div
              v-else
              style="width: 64px; height: 64px; background: #f0f0f0; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #999; font-size: 10px"
            >
              无图
            </div>

            <n-space vertical :size="2">
              <n-text strong>{{ order.title }}</n-text>
...
const disputes = ref<OrderListItem[]>([])
...
function handleArbitrate(orderId: number, decision: string) {
  const label = decision === 'buyer' ? '支持买家' : '支持卖家'
  dialog.warning({
    title: '确认仲裁',
